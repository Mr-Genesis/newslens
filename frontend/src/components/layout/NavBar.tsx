        ...
        <div className="flex items-center justify-between h-[var(--top-bar-height)]">
          ...
          {!isDeepDive && (
            <div className="flex items-center gap-1">
              {/* #100: Feed → /feed (source-type-filtered feed; keeps the 5-tab bottom bar intact) */}
              <Link
                href="/feed"
                aria-current={pathname === "/feed" ? "page" : undefined}
                className={cn(
                  "flex items-center justify-center w-10 h-10 rounded-full transition-colors hover:bg-[var(--surface-raised)]",
                  pathname === "/feed"
                    ? "text-[var(--accent)]"
