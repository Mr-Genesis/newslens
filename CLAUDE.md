...
4. **Clustering** (every 10 min) → pgvector cosine distance (threshold 0.15) → story_clusters table; a new cluster is **eagerly summarized** in the background on creation (`summarizer.schedule_summary`, gated by `EAGER_SUMMARIES_ENABLED`) so the read paths rarely hit a cold summary
5. **Entity Extraction Backfill** (every 15 min, on by default via `GRAPH_EXTRACTION_ENABLED`; needs a platform LLM key, skips gracefully without one) → LLM extraction over settled clusters → entities / entity_aliases / article_entities (G1)
   - **Filing ingest** (in the RSS fetcher, gated by `EXCHANGE_FILINGS_ENABLED`): a `credibility_meta.filing_watchlist` source (the 4 NSE per-company feeds) is an exchange firehose reduced at ingest to only companies in the AGGREGATE watchlist (`filings.aggregate_watch_keys`, union across all users — empty ⇒ nothing ingested), then each kept item is **deterministically** attached to its `org` entity (`filings.attach_filing_entity`, no LLM — the company is the verbatim `<title>`). See "Filing tier" in Key Patterns.
...
- **Filing tier (Phase 3 — India exchange filings):** the `filing` SourceType is watchlist-only (`audience=[]` ⇒ the source-gate hides it). One firehose source carries thousands of companies, so per-source follow can't express "only my companies"; instead `filings.norm_company()` — a suffix-stripped, punctuation-free name key — is the SINGLE key shared by the ingest filter, the entity alias (stored at attach), and the read-path resolution, so they can't drift. NSE feeds carry NO stock symbol (verified live), so matching is by normalized company NAME. Visibility is per-ARTICLE: `routes.get_feed` OR-branches `and_(source_type==filing, entity ∈ filings.watchlisted_entity_ids(caller))` onto the source-gate — empty watchlist/follows (or `EXCHANGE_FILINGS_ENABLED=false`) ⇒ byte-identical feed. Filings appear on the company cast strip via the deterministic attach. The dead NSE/BSE "Corporate Announcements" firehoses are intentionally NOT seeded; EDGAR (US, per-CIK sources) is a later phase that reuses this read-path.
...
- **Persona: depth + locale (WS-7):** `depth_pref` (brief/standard/expert) drives BOTH the retrieval budget (`retrieval.budget_for` = 800/2500/16000 chars/source) AND an answer-style suffix (`lenses._depth_suffix`) across EVERY lens (analysis/ask/frameworks/consensus/strategic/trivia); non-standard depths get a `:depth`-scoped cache subkey so a brief and an expert answer never cross-serve (standard keeps the plain subkey, so `consensus` still feeds `cluster_coherence`). **Region vs locale:** `Source.region` (`"in"`/`"global"`) is the SOURCE's provenance; `User.locale` (`"IN"`, defaults to IN) is the READER's home. `ranking.locale_mult` is a bounded ×[1.0, `locale_affinity_boost`=1.1] feed nudge when they match (lower-cased) — applied ONLY when `LOCALE_AFFINITY_ENABLED` AND the user saved a profile (`persona_version > 1`), so it's a real persona feature, not a global default-IN re-rank. Off/ungated → ×1.0 no-op; the ×1.1 ceiling + kill-switch are the guards (India-first is intended, not a bug).
...
- **Graceful degradation:** LLM generation degrades by provider — a user with any one valid provider key keeps working if another is down. Summaries are **non-blocking** (PR #128): `get_cluster` / `get_briefing` return a `summarizer.snippet_summary()` fallback instantly and schedule the real summary in the background (`schedule_summary` — deduped per cluster, gated by `EAGER_SUMMARIES_ENABLED`, eager on cluster creation), so the read paths never block on the LLM; the snippet is **never persisted** so `backfill_summaries` still generates the real one. If embeddings lag, articles ingest without them and fall back to snippet discovery.
...
- **Client cache (SWR, PR #127):** `frontend/src/lib/cache.ts` — a two-tier stale-while-revalidate cache (in-memory `Map` + IndexedDB via `idb-keyval`) behind the `useCachedResource` hook. The briefing, story detail, and feed page-1 **paint the last-known response instantly** then revalidate in the background — masks the free-tier cold start and kills refetch-on-back-navigation. Per-namespace TTL (briefing 6h / cluster 2h / feed 30m) + oldest-first count/byte eviction (`trimMemory` bounds memory even if the IDB write rejects); a monotonic run token drops a superseded old-key revalidate (no cross-story torn read); `useInfiniteFeed` seeds page-1 + replace-writes-through (WS-3 cursor logic intact, `loadMore` guarded until the cursor is pinned); `usePrefetchClusters` warms the top-3 story details. Degrades to memory-only under SSR/jsdom.
