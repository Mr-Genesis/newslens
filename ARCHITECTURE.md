...
| Client data cache (PR #127) | Two-tier stale-while-revalidate (in-memory `Map` + IndexedDB via `idb-keyval`), per-namespace TTL + oldest-first eviction | No cache (refetch on every mount) / TanStack Query | Paints last-known content instantly + masks the free-tier cold start; bespoke keeps the bundle small and matches the no-data-lib codebase; IDB is proven in this WebView (Firebase uses it). A per-run token prevents a superseded old-key revalidate from cross-serving |
| Cluster summaries (PR #128) | Non-blocking read paths + eager generation on cluster creation (snippet fallback returned instantly, real LLM summary in the background) | Blocking on-demand generation during `/briefing` + `/clusters` | A cold briefing used to stack ~8 blocking LLM calls (24-64s) on top of the cold start; the snippet fallback is never persisted so backfill still generates the real summary |
